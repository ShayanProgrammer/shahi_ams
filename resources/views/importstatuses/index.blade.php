            ...
            <div class="row">
                ...
                <div class="col-3 d-flex align-items-end justify-content-end">
                    ...
                    <div class="btn-group ms-1 check-all-container">
                        ...
                        <a href="{{ route('create_importstatus') }}" class="btn btn-outline-primary btn-icon btn-icon-start ms-0 ms-sm-1 w-100 w-md-auto">
                            <span>Add Import Status</span>
...
{{--                        <button type="button" class="btn btn-primary">Save changes</button>--}}
        ...
        <script>
                    ...
                    success: result => {
                        ...
                        $('#ajax_table').DataTable({
                            ...
                            columns: [
                                ...
                                {{--        //var edit_btn = '<a href="{{ url('importstatuses/edit') }}/'+full['id']+'" class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1">Edit</a>';--}}

                                {{--        var total = full['rate']*full['quantity'];--}}
            ...
            $(document).on('click','.view_detail_btn',function(){
                var import_status_id = $(this).data('id');

                $.ajax({

                    url: "{{ url('import_status_detail/view') }}",
                    method: 'POST',
                    data: {
                    ...
                    success: function(result){
                        ...
                        var html = '';
                        $.each(parsed.import_status_detail, function(k, v) {
                            html += '<tr>';
                            html += '<td>' + v.length + '</td>';
                            html += '<td>' + v.size + '</td>';
                            html += '<td>' + v.rate + '</td>';
                            html += '<td>' + v.quantity + '</td>';
                            html += '<td>' + v.rate*v.quantity + '</td>';
                            html += '</tr>';
                        });
