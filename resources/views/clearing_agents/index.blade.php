            ...
            <div class="row">
                ...
                <div class="col-3 d-flex align-items-end justify-content-end">
                    ...
                    <div class="btn-group ms-1 check-all-container">
                        ...
                        <a href="{{ route('create_clearing_agent') }}" class="btn btn-outline-primary btn-icon btn-icon-start ms-0 ms-sm-1 w-100 w-md-auto">
                            <span>Add Clearing Agent</span>
        ...
        <script>
            ...
            function fetchList(){
                ...
                $.ajax({
                    ...
                    success: result => {
                        ...
                        console.log(data);
                        // return false;
                        var _data = data.clearing_agents;
                        ...
                        $('#ajax_table').DataTable({
                            ...
                            aaSorting: [[ 0, "desc" ]],
                            columns: [
                                {data: 'name', title: 'Clearing Agent Name'},
                                {data: 'phone', title: 'Phone'},
                                {
                                    title: 'Action',
                                    render: function (data, type, full, meta) {
                                        var edit_btn = '<a href="{{ url('clearing_agents/edit') }}/'+full['id']+'" class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1">Edit</a>';
                                        ...
                                        @if($user->role_id == 1)
                                            var account_btn = '<a href="{{ url('clearing_agents/clearing_agent_account') }}/'+full['id']+'" class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1">Accounts</a>';
                                        @else
                                            var account_btn = "";
                                        @endif

                                        return edit_btn + " " + delete_btn + " " + account_btn ;
                                    }
                                }
                            ],
                            dom: 'Blrtip',
            ...
            Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var id = $(this).data('id');
                        $.ajax({
                            url: "{{ route('delete_clearing_agent') }}",
                            data: {
                                id: id
                            },
                            method: 'POST',
